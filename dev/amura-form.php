<script type="text/javascript" data-tag="selldo">
	...
	window.sell_do_form_rendered = function(){
		<?php 
		$json = @file_get_contents('http://1.sell.do/hiranandani_units.json');
	...
	jQuery.ajax({
			...
			success: function(one, two, three){
				...
				jQuery('div.form-group').find('label').each(function(){
					/*if(jQuery(this).text()=='Project'){
						jQuery(this).after("<input type='hidden' name='sell_do[form][lead][project_id]' class='form-control' value='552f641b3bb2f86d1f000406' />");
					}*/
					jQuery(this).hide();
				});
				...
				jQuery('[name="sell_do[form][lead][project_id]"]').parent().after('<div class="form-group">' +  select_field + '<span class="cost unit_configuration_cost" style="display:none;"></span></div>');
				jQuery('[name="sell_do[form][lead][project_id]"]').change(function(){
					var html = "<option value=''>Select Unit configuration</option>";

					// project value is added manually for property specific page
					var data = units_data[jQuery(this).val()];
					// var propertyName = 'Edenhall';
					// var data = units_data['552f641b3bb2f86d1f000406'];

					if(typeof data !== "undefined"){
						for(i=0;i<data.length;i++){
							// filter unwanted properties from dropdown
							var noPropertyArray = new Array();
							noPropertyArray = ['Edenhall', 'Cross Gate', 'Chancery', 'Oceanic', 'Birchwood', 'Greenwood', 'Brentwood', 'Pinewood','Seawood'];
							var str = data[i].name;
							var PropArray = str.split('-');
							var Prop = jQuery.trim(PropArray[0]);
							console.log(Prop+'----'+jQuery.inArray(Prop, noPropertyArray));
							if(jQuery.inArray(Prop, noPropertyArray) == -1){
								html += "<option data-cost='" + data[i].cost + "' value='" + data[i].id + "'>" + data[i].name + "</option>";
							}
							
						}
						jQuery('[name="sell_do[form][lead][unit_configuration_id]"]').html(html);
						jQuery('[name="sell_do[form][lead][unit_configuration_id]"]').val("");
						jQuery('[name="sell_do[form][lead][unit_configuration_id]"]').removeAttr("required");
						if(jQuery('[name="sell_do[form][lead][project_id]"]').val() == ""){
							jQuery('[name="sell_do[form][lead][unit_configuration_id]"]').closest(".form-group").hide();
						}else{
							jQuery('[name="sell_do[form][lead][unit_configuration_id]"]').closest(".form-group").show();
							jQuery('[name="sell_do[form][lead][unit_configuration_id]"]').attr("required", "required");
						}
					}else{
						jQuery('[name="sell_do[form][lead][unit_configuration_id]"]').closest(".form-group").hide();
					}
				});
				jQuery('[name="sell_do[form][lead][unit_configuration_id]"]').change(function(){
					if(jQuery(this).val() != ""){
						jQuery(".unit_configuration_cost").show();
						jQuery(".unit_configuration_cost").html("Cost: Rs. " + formatMoney(jQuery(this).find("option:selected").attr("data-cost")));
					}else{
						jQuery(".unit_configuration_cost").hide();
					}
				});
...
jQuery(window).load(function(){
	...
	jQuery('.btn').click(function(){
		str = jQuery( "form.sell_do_form_actual_container" ).serialize();
		// console.log(str);
	});
});
</script>