...
<div class="panelcont">
	<h3 class="green">Brands we do</h3>
		...
		<marquee behavior="scroll" onmouseover="this.stop();" onmouseout="this.start();"  scrolldelay="120" style="width:100%"  align="middle">';
		$result=mysql_query('SELECT brandname,brandimg FROM `brand` order by brandid desc ');
		while($row=mysql_fetch_array($result)) {echo '
<img src="images/brands/thumb/'.$row['brandimg'].'" alt="'.$row['brandname'].'" class="marl10 mart10">';
		}
		echo '</marquee>
		</div>
</div>
